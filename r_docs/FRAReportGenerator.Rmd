...
```{r receipt}
receipt <- list()
receipt$causation <- list()
receipt$causation$rollover <- list(
  fpar(
    ftext(
  ...
  fpar(
    ...
    ftext(". I have reviewed the documentation accompanying your correspondence including medical records, information regarding the subject crash, litigation documents, and other materials, including the "),
    ftext(
      if (defense_biomech_expert$depo_reviewed == "yes")
        paste0(convert_date_format(defense_biomech_expert$depo_date), " deposition transcript and the ")),
    ftext(paste0(convert_date_format(defense_biomech_expert$report_date), 
...
doc %>%
  ...
  block_add_fun((receipt$causation$rollover)) %>%
